...
// テスト用の具体的な実装クラス
class TestCommandHandler extends commandBaseHandler_1.CommandBaseHandler {
    constructor(env, lineService, 
    ...
    async handleMessage(replyToken, message, 
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    userId) {
        try {
            // ヘルプコマンドの処理
            if (message.startsWith('ヘルプ')) {
                ...
                if (helpCommand === '') {
                    // 全コマンドのヘルプ
                    await this.lineService.replyText(replyToken, '以下のコマンドが利用可能です：\n- 家事管理\n- 家計簿\n- 買い出しリスト');
                    return;
                }
                else if (helpCommand === '家事管理') {
                    // 特定のコマンドのヘルプ
                    await this.lineService.replyText(replyToken, '家事管理コマンドの詳細な使い方を説明します。');
                    return;
                }
            }
            // 既存のコマンド処理ロジック
    ...
    // デフォルトのメッセージハンドラー
    async handleDefaultMessage(replyToken, message) {
...
describe('CommandBaseHandler', () => {
    ...
    describe('parseCommand', () => {
        it('should parse 家事管理 command correctly', () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const parseMethod = testHandler.parseCommand;
    ...
    describe('handleMessage', () => {
        it('should handle 家事管理 command', async () => {
            ...
            expect(mockLineService.replyText).toHaveBeenCalledWith(replyToken, '家事管理コマンドを処理');
        });
        it('should handle 家計簿 command', async () => {
            const replyToken = 'test-reply-token';
            await testHandler.handleMessage(replyToken, '家計簿');
            expect(mockLineService.replyText).toHaveBeenCalledWith(replyToken, '家計簿コマンドを処理');
        });
    ...
    describe('help generation', () => {
        it('should generate help text for a single command', () => {
            const helpMethod = testHandler.generateHelp;
        ...
        it('should handle help request command', async () => {
            ...
            expect(mockLineService.replyText).toHaveBeenCalledWith(replyToken, expect.stringContaining('以下のコマンドが利用可能です'));
        });
        it('should provide detailed help for specific command', async () => {
            ...
            expect(mockLineService.replyText).toHaveBeenCalledWith(replyToken, expect.stringContaining('家事管理コマンドの詳細な使い方'));
        });
    });
    describe('error handling', () => {
        it('should handle errors gracefully', async () => {
            ...
            const errorTestHandler = new TestCommandHandler(mockEnv, mockLineService, errorHandler);
        ...
        it('should log error details', async () => {
            ...
            const errorTestHandler = new TestCommandHandler(mockEnv, mockLineService, errorHandler);
            await errorTestHandler.handleMessage(replyToken, '家事管理');
            expect(consoleSpy).toHaveBeenCalledWith('Error in command handler:', testError);
    ...
    describe('main method', () => {
        it('should call run method and log end message', async () => {
            ...
            const runSpy = jest.spyOn(testHandler, 'run');
        ...
        it('should call notice method when run method throws an error', async () => {
            ...
            const errorTestHandler = new TestCommandHandler(mockEnv, mockLineService, errorHandler);
            const noticeSpy = jest.spyOn(errorTestHandler, 'notice');
            const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
