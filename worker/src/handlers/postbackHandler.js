...
class PostbackHandler {
    constructor(env) {
        ...
        this.lineService = new lineMessaging_1.LineMessagingService(env);
        this.sheetsService = new googleSheets_1.GoogleSheetsService(env);
        this.purchaseHandler = new purchaseHandler_1.PurchaseHandler(this.sheetsService, this.lineService);
        // 初期化処理を実行
        this.initialize();
    }
    /**
     ...
     * ポストバックを処理する
     */
    async handlePostback(replyToken, data, userId) {
     ...
     * 家事管理のポストバックを処理する
     */
    async handleHouseworkPostback(action, userId) {
        switch (action) {
            case 'report':
     ...
     * 家計簿のポストバックを処理する
     */
    async handleAccountBookPostback(action) {
        const dt = new Date();
     ...
     * 家事の状況を取得する
     */
    async getHouseworkStatus(_userId) {
        try {
     ...
     * 買い物リストのポストバックを処理する
     */
    async handlePurchasePostback(action) {
        switch (action) {
            case 'list':
